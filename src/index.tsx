...
function renderStatusLabel(status?: string) {
  ...
  if (lowerStatus === 'ready' || lowerStatus === 'enforcing' || lowerStatus === 'true') {
    ...
    lowerStatus === 'waiting-for-identity' ||
    lowerStatus === 'waiting-to-regenerate' ||
    lowerStatus === 'restoring' ||
    lowerStatus === 'regenerating' ||
    lowerStatus === 'pending'
  ) {
    severity = 'warning'; // Treat pending as warning
  } else if (
    lowerStatus === 'disabled' ||
    lowerStatus === 'non-enforcing' ||
    lowerStatus === 'no status'
  ) {
...
function formatCIDRRule(rule: any): string {
  let text =
    rule.cidr || rule.cidrGroupRef || formatSelector(rule.cidrGroupSelector) || 'Invalid Rule';
...
function formatPortRule(portRule: any): React.ReactNode {
  ...
  const ports = portRule.ports
    ?.map((p: any) => `${p.port || '*'}/${p.protocol || 'ANY'}`)
    .join(', ');
  const hasL7 = !!(
    portRule.rules?.http ||
  ...
  return (
    <>
      ...
      <SectionBox title="Identity Labels">
        <NameValueTable
          rows={
            identity?.labels?.map((label: string) => ({ name: label })) || [
  ...
  return (
    <>
      ...
      <SectionBox title="Health Status">
        <NameValueTable
          rows={Object.entries(statusHealth || {}).map(([k, v]) => ({ name: k, value: String(v) }))}
        />
      </SectionBox>
      <SectionBox title="Encryption Status">
        <NameValueTable
          rows={Object.entries(statusEncryption || {}).map(([k, v]) => ({
            name: k,
            value: String(v),
          }))}
        />
    ...
    <ResourceListView
      ...
      columns={[
        ...
        'namespace',
        {
          id: 'identity',
          label: 'Identity ID',
          getter: (ep: KubeObjectInterface) => ep.status?.identity?.id ?? '-',
          sort: true,
        },
        // Add getter to state column
        {
          ...
          cellProps: (ep: KubeObjectInterface) => ({
            children: renderStatusLabel(ep.status?.state),
          }), // Custom rendering
          sort: true,
        },
        {
          id: 'ipv4',
          label: 'IPv4',
          getter: (ep: KubeObjectInterface) => ep.status?.networking?.addressing?.[0]?.ipv4 || '-',
        },
        {
          id: 'ipv6',
          label: 'IPv6',
          getter: (ep: KubeObjectInterface) => ep.status?.networking?.addressing?.[0]?.ipv6 || '-',
        },
        'age',
      ]}
    />
    ...
    <ResourceListView
      ...
      columns={[
        'name',
        {
          ...
          getter: (id: KubeObjectInterface) =>
            Object.entries(id.jsonData?.['security-labels'] || {})
              .map(([k, v]) => `${k}=${v}`)
              .join(', '),
        },
        'age',
      ...
      columns={[
        'name',
        {
          ...
          getter: (node: KubeObjectInterface) =>
            node.spec?.addresses?.find((a: any) => a.type === 'CiliumInternalIP')?.ip || '-',
        },
        {
          id: 'internalIP',
          label: 'Internal IP',
          getter: (node: KubeObjectInterface) =>
            node.spec?.addresses?.find((a: any) => a.type === 'InternalIP')?.ip || '-',
        },
        'age',
      ]}
    />
